...
def test_RD_plot_line(
    ...
    combinations = list(itertools.product( params["N"], params["delta"], 
                                        ...
                                        params["nb_iteration"],params["algo_generation"],
                                        params["nbIterationsHCRoaring"],params["nbIterationsHC"],
                                        params["selectionAlgorithme"]))
    ...
    headers = ["Benchmark name","N","delta", 
                "y","alpha","beta",
                "nb_iteration","algo_generation","nbIterationsHCRoaring","nbIterationsHC",
                "selectionAlgorithme",
    ...
    for bench_name, bench_size, bench_opt in zip(benchs["name"], benchs["nb_nodes"], benchs["OPT"]):
        if bench_name in benchmarks:
            ...
            for combination in combinations:
                (N,delta,y,alpha,beta,nb_iteration,algo_generation,nbIterationsHCRoaring,nbIterationsHC,selectionAlgorithme) = combination
                print("      Params : ",end =' ')
                print((N,delta,y,alpha,beta,nb_iteration,algo_generation.__name__,nbIterationsHCRoaring,nbIterationsHC,selectionAlgorithme.__name__))     
                cumul_time = 0
                cumul_result = 0
                min_result = INF 
                nb_opt = 0
                ...
                for i in range(nb_executions):
                    start = time.time()
                    opt_circuit,opt_cost  = RED_DEERS(
                                G,
                                N,
                                delta,
                                y,
                ...
                writer.writerow([bench_name,N,delta,y,alpha,beta,nb_iteration,algo_generation.__name__,nbIterationsHCRoaring,nbIterationsHC,selectionAlgorithme.__name__,"%.3f" % avg_perc,min_result,nb_opt,"%.4f" % avg_time])
...
if  __name__ == "__main__" :
    ...
    benchmarks = ["ftv33","ftv38","ftv47","ftv70","ftv90","rbg443","rbg403","kro124p"]
    ...
    params = {
            ...
            "nbIterationsHCRoaring":[200],    # Roaring parameter : number of iterations 
    ...
    test_RD_plot_line(benchmarks,params,file_name = './results/RD_alone_test_final.csv',nb_executions = 5,
            param_name = "delta",plot = False,plot_name='./results/RD_alone_test_plot')
