...
def test_algorithm(aglorithm, SIGMA0, ALPHA, BETA, Q, P, NB_ITER, max_nodes=1000, nb_executions=1, filename='./results/results.csv'):
    """
    ...
    for bench_name, bench_size, bench_opt in zip(benchs["name"], benchs["nb_nodes"], benchs["OPT"]):
        if (bench_size <= max_nodes):
            try:
                G, n = load_benchmark(
                    f"../../Heuristics/Benchs/{bench_name}.atsp")
            except:
